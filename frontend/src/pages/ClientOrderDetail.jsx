...
export default function ClientOrderDetail() {
  ...
  useEffect(() => {
    api.getClientOrderById(id).then(setOrder).catch(() => {});
    api.getClientOrderItems(id).then(data => setItems(Array.isArray(data) ? data : [])).catch(() => {});
  }, [id]);

  async function handleConfirm() {
    setActionLoading(true);
    try {
      const updated = await api.confirmClientOrder(id);
      setOrder(updated);
    } catch {}
  ...
  async function handleReject() {
    ...
    setActionLoading(true);
    try {
      const updated = await api.rejectClientOrder(id);
      setOrder(updated);
    } catch {}
    setActionLoading(false);
  }
    ...
    <Layout>
        ...
        {order.status === 'new' && (
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 10 }}>
            <button onClick={handleConfirm} disabled={actionLoading}
              style={{ padding: '8px 20px', background: '#10b981', color: '#fff', border: 'none', borderRadius: 7, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
              ✓ Подтвердить
            </button>
            <button onClick={handleReject} disabled={actionLoading}
              style={{ padding: '8px 20px', background: '#ef4444', color: '#fff', border: 'none', borderRadius: 7, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
              ✕ Отклонить
            </button>
          </div>
        )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
        <InfoCard title="Информация о заказе">
          ...
          <InfoRow label="Дата" value={order.order_date?.slice(0, 10)} />
          <InfoRow label="Источник" value={order.source || '—'} />
          <InfoRow label="Способ доставки" value={order.delivery_method || '—'} />
        </InfoCard>
        <InfoCard title="Клиент и финансы">
          <InfoRow label="Клиент" value={order.client_name || '—'} />
          <InfoRow label="Сумма заказа" value={`${order.total_amount?.toLocaleString('ru')} ₽`} />
        </InfoCard>
      </div>

      <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 1px 6px rgba(0,0,0,0.07)', padding: '20px 24px' }}>
        <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 14 }}>Состав заказа</div>
        {!items.length ? (
